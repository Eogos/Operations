...
_vP = getposATL vehicle (([] CALL AllPf) call RETURNRANDOM);
...
_start = + _start2;
_dir = [_locationA, _start] call BIS_fnc_dirTo;
...
[[_mid],"mil_arrow","ColorRed",([_start, _locationA] call BIS_fnc_dirTo)] SPAWN SAOKCREATEMARKER;
};
...
waitUntil {sleep 5; {(vehicle _x) distance _locationA < 1400} count ([] CALL AllPf) > 0 || {_timer < time}};
if !(_timer < time) then {
_group = [_start,_locationA,50] CALL SAOKCSATBOATINF;
_INFgroups set [count _INFgroups,_group];
_group = [_start,_locationA,50] CALL SAOKCSATBOATINF;
_INFgroups set [count _INFgroups,_group];
_group = [_start,_locationA,50] CALL SAOKCSATBOATINF;
_INFgroups set [count _INFgroups,_group];
...
[[_mid],"mil_arrow","ColorRed",([_start, _wpPP] call BIS_fnc_dirTo)] SPAWN SAOKCREATEMARKER;
};
{
...
_time = time + 600;
_text = ("All units near "+_Lna+", be aware. CSAT is heading at the beach now. Out");
[[BaseR, _text],"SAOKMULTIGCHAT",nil,false] spawn BIS_fnc_MP;
...
[["CSAT Captured the Beach",WEST],"HINTSAOK",nil,false] spawn BIS_fnc_MP;
...
[_locationA,WEST] SPAWN ADDR;
[["CSAT Assault on Beach Failed",WEST],"HINTSAOK",nil,false] spawn BIS_fnc_MP;
...
//FAR W T
_time = time + 600;
_text = ("Be aware, CSAT is heading now at the beach near "+_Lna+". Out");
[[BaseR, _text],"SAOKMULTIGCHAT",nil,false] spawn BIS_fnc_MP;
waitUntil {sleep 5; _time < time || {{(vehicle _x) distance _locationA < 1400} count ([] CALL AllPf) > 0}};
if !(_timer < time) then {
//CLOSE
_group = [_start,_locationA,50] CALL SAOKCSATBOATINF;
_INFgroups set [count _INFgroups,_group];
...
_time = time + 600;
_text = ("CSAT is getting very close the beach near "+_Lna+". All nearby units, assist if you can. Out");
[[BaseR, _text],"SAOKMULTIGCHAT",nil,false] spawn BIS_fnc_MP;
_ker = (count _ALLunits)*0.2;
waitUntil {sleep 5; {alive _x && {isNil{_x getvariable "SaOkSurrendered"}} && {!(fleeing _x)}} count _ALLunits < _ker || {_time < time}};
if (_time < time) then {
