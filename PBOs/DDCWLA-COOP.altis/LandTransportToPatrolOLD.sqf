private ["_start","_LZ","_infwaypoint","_side","_dir","_chopclass","_unitclasses","_skill","_num","_startC","_chop","_groupLTTP","_nul","_wrn","_wp1"];
...
waitUntil {sleep 3; isNil"_chop" || {isNil"_groupLTTP"} || {((_chop select 0) distance _LZ <400)} || {!(alive (_chop select 0))} || {!(alive driver (_chop select 0))} || {({alive _x} count crew (_chop select 0)) < 1} || {behaviour leader (_chop select 2) == "COMBAT"}};
...
{_nul = [_x,(_chop select 0)] SPAWN FJumpOffCar;} foreach units _groupLTTP;
...
waitUntil {sleep 3; isNil"_chop" || {isNil"_groupLTTP"} || {(_chop select 0) distance _start < 40} || {!(alive (_chop select 0))} || {!(alive driver (_chop select 0))} || {({alive _x} count crew (_chop select 0)) < 1} || {behaviour leader (_chop select 2) == "COMBAT"}};
if (isNil"_chop" || {isNil"_groupLTTP"}) exitWith {DONTDELGROUPS = DONTDELGROUPS - [_groupLTTP];};
if (!(alive (_chop select 0)) || {!(alive driver (_chop select 0))} || {({alive _x} count crew (_chop select 0)) < 1}) exitWith {
DONTDELGROUPS = DONTDELGROUPS - [_groupLTTP];
if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};
