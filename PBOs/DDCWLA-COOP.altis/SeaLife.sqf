...
while {count _units > 80} do {sleep 7; {if (_x distance (vehicle player) > 120) then {_units = _units - [_x];deletevehicle _x;};} foreach _units;};
_Ppos = (getposATL (vehicle player));
_start = [(_Ppos select 0) - 50 + random 100, (_Ppos select 1) - 50 + random 100, -10+(random 9)];
while {!surfaceIsWater _start || [_start,(vehicle player)] CALL FUNKTIO_LOSOBJ} do {sleep 0.1;_start = [(_Ppos select 0) - 50 + random 100, (_Ppos select 1) - 50 + random 100, -10+(random 9)];}; 
