...
private ["_xx"];
_xx = _allU select _i;
if (typeof _xx iskindof "CIVILIAN" && {alive _xx} && {!(_xx in _unitsAICAMPC)} && {isNil{_xx getvariable "CivNo"}} && {vehicle _xx == _xx}) then {
_d = _xx SPAWN _fafas;
...
{
		if (isNull _x || {!alive _x}) exitWith {_unitsAICAMPC = [_unitsAICAMPC,_forEachIndex] call BIS_fnc_removeIndex;};
sleep 0.01;
} foreach _unitsAICAMPC;
{if (isNil"_x" || {typename _x != "OBJECT"} || {!(typeof _x iskindof "Man")}) exitWith {_unitsAICAMPC = [_unitsAICAMPC,_forEachIndex] call BIS_fnc_removeIndex;};sleep 0.01;} foreach _unitsAICAMPC;
sleep 1;
_c = count _unitsAICAMPC - 1;
for "_i" from 0 to _c do {
private ["_xx"];
_xx = _unitsAICAMPC select _i;
if (!isNull _xx && {behaviour _xx == "DANGER"} && {random 1 < 0.3} && {alive _xx}) then {
_ani = ["AmovPercMstpSnonWnonDnon_Scared","AmovPercMstpSnonWnonDnon_Scared2"] call BIS_fnc_selectRandom;
//_xx playmove _ani;
[[_xx,_ani],"SAOKPMOVE",nil,false] spawn BIS_fnc_MP;
} else {
