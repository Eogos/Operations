...
if ("USHelp" in (SaOkmissionnamespace getvariable "Progress")) then {lbAdd [1500, "Artillery"];};
lbAdd [1500, "Gear Drop"]; 
if ("USHelp" in (SaOkmissionnamespace getvariable "Progress")) then {lbAdd [1500, "Support"];};
//lbAdd [1500, "Diversions"]; 
lbAdd [1500, "Vehicles [Para]"]; 
lbAdd [1500, "Teammates [Para]"]; 
if (!isNil"CIVMODE") then {lbAdd [1500, "Undercover"];};
if (isNil"IFENABLED" && {("GreenHelp" in (SaOkmissionnamespace getvariable "Progress") || {"USHelp" in (SaOkmissionnamespace getvariable "Progress")} || {"MilOff" in (SaOkmissionnamespace getvariable "Progress")})}) then {
...
while {dialog} do {
	...
	if ("GreenHelp" in (SaOkmissionnamespace getvariable "Progress")) then {
	...
	if ("GreenHelp" in (SaOkmissionnamespace getvariable "Progress") || {"USHelp" in (SaOkmissionnamespace getvariable "Progress")} || {"MilOff" in (SaOkmissionnamespace getvariable "Progress")}) then {lbAdd [1501, "Chopper Transport"];};
	lbSetCurSel [1501, 0];
	};
	...
	_text = "Price: ";
	_cost = if ((lbText [1500, (lbCurSel 1500)]) == "Teammates [Para]") then {100} else {((lbText [1501, (lbCurSel 1501)]) CALL SUPPORTCOST)};
	...
	if (_unitC != "") then {
	if (count units group player < 7) then {
	if (([side player] CALL PrestigeS) >= _cost) then {
	_n = [side player,-_cost] SPAWN PrestigeUpdate;
	"Cash" SPAWN SAOKPLAYSOUND;
	[[-_cost, "Received Team-mate"],"PRESTIGECHANGE",nil,false] spawn BIS_fnc_MP;
	_bs = 0.47;
	[_unitC,_bs,"PARA"] SPAWN SAOKUADD;
	} else {(format ["%1 more prestige value needed to receive new team-mate", _cost - ([side player] CALL PrestigeS)]) SPAWN HINTSAOK;};
	} else {"You are not allowed to have more team-mates" SPAWN HINTSAOK;};
	};
	};
	if ((lbText [1500, (lbCurSel 1500)]) == "Undercover") then {
	...
	if (count _waypoints > 0) then {SaOkmissionnamespace setvariable ["NewExitZ",(_building buildingPos (_waypoints select 0))];};
	_range = _range + 100;
	sleep 0.1;
	};
	...
	if (isNil{SaOkmissionnamespace getvariable "Chopped"}) then {
	_n = [side player,-_cost] SPAWN PrestigeUpdate;
	"Cash" SPAWN SAOKPLAYSOUND;
	[[-_cost, "Support Call"],"PRESTIGECHANGE",nil,false] spawn BIS_fnc_MP;
