...
_nVil = getmarkerpos ([] CALL NEARESTVILLAGE);
[
WEST, // Task owner(s)
...
[_someId, "onEachFrame", {
	...
	drawIcon3D ["\A3\ui_f\data\map\markers\military\join_CA.paa", ICONCOLORBLUE, _this, 0.8, 0.8, 0, (format ["Fight with Locals: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
...
waitUntil {sleep 10; {getmarkercolor _x == "ColorBlue"} count AmbientZonesN >= 3 || {{getmarkercolor _x in ["ColorPink","ColorGreen"]} count FacMarkers >= 2}};
...
waitUntil {sleep 10; pisteet > 4000 || {count AmbientZonesN > 10 && {((({getmarkercolor _x == "ColorBlue"} count AmbientZonesN) * 4) >= {getmarkercolor _x == "ColorRed"} count AmbientZonesN)}}};
...
_li2 = "Excelent, I let the HQ know";
};
_n = [
[getposATL player, player, _actor1],
[],
...
,[[[_actor1], ["Next assigment for you to do is to capture at least 2 piers for us. That makes it easier to bring our gear in"],11]]
,[[[player], _li,6]]
,[[[_actor1], [_li2],5]]
]
] SPAWN SAOKCUTSCENE;
...
["Lieutenant, excelent work - your succesfull operation on the island is over. Head to the extraction point and call pick up via 0-0-9.", "Complete Operation with 0-0-9", "Complete Operation with 0-0-9"], // Task description
_start, // Task destination
true // true to set task as current upon creation
] call SAOKCRTASK;
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\destroy_CA.paa", ICONCOLORRED, getmarkerpos _this, 0.8, 0.8, 0, (format ["Disable Mortar: %1m",round (getmarkerpos _this distance player)]), 1, SAOKFSI, "TahomaB"];
...
["Resistance is asking for our assistance to deal with CSAT mortars that have been spotted on borderline.", "Disable CSAT Mortars On Borderline", "Disable CSAT Mortars On Borderline"], // Task description
...
sleep 30;
_c = "";
{if (getmarkercolor _x == "ColorBlue") exitWith {_c = _x;};} foreach AmbientZonesN;
if (_c == "") exitWith {SAOKDYNTASKS set [count SAOKDYNTASKS, ["SAOKPIERT"]];};
gameLogic1 globalchat localize "STR_milT4_l1";
sleep 9;
player sidechat localize "STR_milT4_l2";
_ps = + PierMarkers;
_ps = [_ps,[getmarkerpos _c],{(_this select 0) distance getmarkerpos _x},"ASCEND"] call BIS_fnc_sortBy;
_ps = _ps select 0;
_d = 40;
...
["Resistance is asking for our assistance to deliver 4 crate shipment from pier to their camp.<br/><br/><img image='arma3pier.jpg' width='360' height='202.5'/>", "Transport 4 Crates from Pier to Camp", "Transport 4 Crates from Pier to Camp"], // Task description
//[21857.4,10972.3,0], // Task destination
true // true to set task as current upon creation
] call SAOKCRTASK;
NUMM=NUMM+1;
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\pickup_CA.paa", ICONCOLORBLUE, _this, 0.8, 0.8, 0, (format ["Pick Up Crates Here: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
...
[_someId2, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\marker_CA.paa", ICONCOLORBLUE, _this, 0.8, 0.8, 0, (format ["Deliver Crates Here: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
...
_di= [_startC, _pPos] call BIS_fnc_dirTo;
...
while {surfaceiswater _start} do {sleep 3; _start = [(vehicle player),5000,2000,"(1 - houses) * (1 - sea)"] CALL SAOKSEEKPOS;};
...
["You have been given an order to search blackfoot team's crashsite.", "Search Blackfoot Crashsite", "Search Blackfoot Crashsite"], // Task description
//[21857.4,10972.3,0], // Task destination
true // true to set task as current upon creation
] call SAOKCRTASK;
_someId = format ["IDSAOK%1",NUMM];
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\unknown_CA.paa", ICONCOLORBLUE, _this, 0.8, 0.8, 0, (format ["Search Crashsite: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
...
,[[[_actor1], ["They were taken and transported away many moments ago by group of CSAT soldiers"],10]]
...
["We are tasked to hold the crashsite until the Blackfoot is taken out by missile", "Guard the Blackfoot Crashsite", "Guard the Blackfoot Crashsite"], // Task description
//[21857.4,10972.3,0], // Task destination
true // true to set task as current upon creation
] call SAOKCRTASK;
_someId = format ["IDSAOK%1",NUMM];
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\marker_CA.paa", ICONCOLORBLUE, _this, 0.8, 0.8, 0, (format ["Guard Crashsite from Safe Distance: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
}, _start] call BIS_fnc_addStackedEventHandler;
_mar = format ["MainTaskM%1",NUMM];
NUMM=NUMM+1;
...
_n = getmarkerpos ([] CALL NEARESTVILLAGE); 
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\a3\Ui_f\data\GUI\Cfg\CommunicationMenu\instructor_ca.paa", ICONCOLORBLUE, _this, 0.8, 0.8, 0, (format ["Question CSAT Captive: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
}, _start] call BIS_fnc_addStackedEventHandler;
_mar = format ["MainTaskM%1",NUMM];
NUMM=NUMM+1;
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\circle_CA.paa", ICONCOLORBLUE, _this, 0.8, 0.8, 0, (format ["Find missing Pilots inside 200m radius: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
}, _start] call BIS_fnc_addStackedEventHandler;
_mar = format ["MainTaskM%1",NUMM];
NUMM=NUMM+1;
...
_nn = 0;
while {{!isNil{_x getvariable "NATO"}} count GuardPosts == 0 && {_nn < 10}} do {
_nn = _nn + 1;
if !(_nn < 10) then {
...
waitUntil {sleep 4; {!isNil{_x getvariable "NATO"}} count GuardPosts > 0};
//waitUntil {sleep 4; {!isNil{_x getvariable "NATO"}} count GuardPosts > 0};
[
WEST, // Task owner(s)
...
_post = "";
{if (!isNil{_x getvariable "NATO"}) exitWith {_post = _x;};} foreach GuardPosts;
...
sleep 1;
_mark = [];
{if (getmarkerpos _x distance _locat < 3000) then {_mark = _mark + [_x];};} foreach VEHZONES;
...
waitUntil {sleep 10; {_x in AllMapMarkers && {getmarkercolor _x == "ColorRed"} && {getmarkerpos _x distance _locat < 3000} && {!surfaceisWater (getmarkerpos _x)}} count _mark == 0};
...
_camps = [];
{if (getmarkercolor _x == "ColorRed" && {player distance getmarkerpos _x > 1800}) then {_camps pushback _x;};} foreach AmbientZonesN;
_Radpos = if (count _camps > 0) then {getmarkerpos (_camps call RETURNRANDOM)} else {[player,7600,1800,"(1 + meadow) * (1 - sea)"] CALL SAOKSEEKPOS};
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\circle_CA.paa", ICONCOLORRED, _this, 0.8, 0.8, 0, (format ["Find Radar inside 500m radius: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\flag_CA.paa", ICONCOLORRED, _this, 0.8, 0.8, 0, (format ["Clear: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
}, _nVil] call BIS_fnc_addStackedEventHandler;
waitUntil {sleep 5; player distance _nVil < 200};
_pat = [_nVil,120,10,"(1 + meadow) * (1 - sea)"] CALL SAOKSEEKPOS;
[_pat, [2,3]] SPAWN SAOKADDPATROL;
...
_pi = + PierMarkers;
_pi = [_pi,[],{player distance getmarkerpos _x},"ASCEND"] call BIS_fnc_sortBy;
_pi = getmarkerpos (_pi select 0);
NUMM=NUMM+1;
_someId2 = format ["IDSAOK%1",NUMM];
[_someId2, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\marker_CA.paa", ICONCOLORRED, _this, 0.8, 0.8, 0, (format ["Drop Reporter Here: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\pickup_CA.paa", ICONCOLORGREEN, getposATL _this, 0.8, 0.8, 0, (format ["PickUp Reporter: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\circle_CA.paa", ICONCOLORBLUE, _this, 0.8, 0.8, 0, (format ["Scout the area inside 200m radius: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
}, _start] call BIS_fnc_addStackedEventHandler;
_mar = format ["MainTaskM%1",NUMM];
NUMM=NUMM+1;
...
_nn = 0;
while {{!isNil{_x getvariable "GREEN"}} count GuardPosts == 0 && {_nn < 10}} do {
_nn = _nn + 1;
if !(_nn < 10) then {
...
//waitUntil {sleep 4; {!isNil{_x getvariable "GREEN"}} count GuardPosts > 0};
_post = "";
{if (!isNil{_x getvariable "GREEN"}) exitWith {_post = _x;};} foreach GuardPosts;
[
WEST, // Task owner(s)
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\pickup_CA.paa", ICONCOLORGREEN, _this, 0.8, 0.8, 0, (format ["Assist: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
...
waitUntil {sleep 2; scriptdone _n};
_nVil = getmarkerpos ([_carP] CALL NEARESTVILLAGE);
_waypoints = [];
_l = 60;
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\circle_CA.paa", ICONCOLORRED, _this, 0.8, 0.8, 0, (format ["Find and Take Out Sniper, inside 100m: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
...
_arr = (_z select 0) CALL SAOKZONEDR;
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\destroy_CA.paa", ICONCOLORRED, getmarkerpos _this, 0.8, 0.8, 0, (format ["Destroy Armor: %1m",round (getmarkerpos _this distance player)]), 1, SAOKFSI, "TahomaB"];
...
while {_random <  (_unitrate select 0)} do {_random = round(random (_unitrate select 1));};
_classes = [];
while {_random > 0} do {_classes set [count _classes,FRIENDC4 call RETURNRANDOM];_random = _random - 1;};
...
[_someId, "onEachFrame", {
	if (isNil"IC3D") exitWith {};
	drawIcon3D ["\A3\ui_f\data\map\markers\military\unknown_CA.paa", ICONCOLORRED, _this, 0.8, 0.8, 0, (format ["Find and Make Captive, inside 100m: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
