...
["ScoreAdded",[_tex,30]] call bis_fnc_showNotification;
pisteet = pisteet + 100;
_tex = "CSAT "+ _ty +" was disabled";
_nul = [100, _tex] SPAWN PRESTIGECHANGE;
} else {
//["ScoreRemoved",["Enemy chopper managed to withdraw unharmed",40]] call bis_fnc_showNotification;
};
}; 
...
waituntil {sleep 5; {vehicle _x distance _unit < 150} count ([] CALL AllPf) == 0};
deletevehicle _unit;
};
...
SAOKSHF7 = {
	...
	waitUntil {sleep 1; isNull _this || {{{isplayer _x} count crew _x > 0 || {(count crew _x > 0 && {side (driver _x) in [CIVILIAN, WEST]})}} count ((getposATL _this) nearEntities [["Car"],30]) > 0}};  
...
(_this select 1) setmarkerpos _p;
};
sleep 0.3;
_s = "";
{if (alive leader _x) exitWith {_s = leader _x;};} foreach _u;
...
SAOKSHF10 = {
waitUntil {sleep 10; {vehicle _x distance _this < 600} count ([] CALL AllPf) == 0 || {{_x in (crew _this)} count ([] CALL AllPf) > 0}}; 
if ({_x in (crew _this)} count ([] CALL AllPf) == 0) then {
deletevehicle _this;
} else {CARS pushBack _this;};
};

SAOKSHF11 = {if ({vehicle _x distance _this < 600} count ([] CALL AllPf) > 0) then {waitUntil {sleep 10; {vehicle _x distance _this < 600} count ([] CALL AllPf) == 0};}; deletevehicle _this;};
...
VEHZONESA = VEHZONESA - [_this]; deletemarker _this;
publicVariable "VEHZONESA";
};

SAOKSHF13 = {
while {{(_this select 0) == _x select 1} count RESERVEAMARKS > 0} do {
((_this select 0)+"a") setmarkerpos (getmarkerpos (_this select 1));(_this select 0) setmarkerpos (getmarkerpos (_this select 1));sleep 1;
};
};
...
private ["_aika"];
_aika = time + 15;
waitUntil {sleep 0.1; _aika < time || {getmarkerpos _this distance [0,0,0] > 40}};
if (_aika < time) then {
...
SAOKSHF16 = {[_this,0] SPAWN VehLife;waitUntil {sleep 2; isNull _this || {count crew _this > 0}};if (!isNull _this) then {[_this,1] SPAWN VehLife;};};
...
_unit = _this select 0;
sleep 60;
waituntil {sleep 10; isNull _unit || {{vehicle _x distance _unit < 700} count ([] CALL AllPf) == 0}};
deletevehicle _unit;
};
...
if !((_chop select 0) distance _LZ < 350) exitWith {if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};{_nul = [_x] SPAWN SAOKSHF18;} foreach (units (_chop select 2));{if (vehicle _x == (_chop select 0)) then { unassignvehicle _x; _x action ["Eject",vehicle _x];};} foreach units _groupCT;DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];CantCommand = CantCommand - [_chop select 2];if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};FriendlyInf set [count FriendlyInf,_groupCT];};
//ELSE NEAR LZ -> JUMP
...
//CHOP DOWN
if (isNil"_chop" || {isNil"_groupCT"}) exitWith {DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];};
if !((_chop select 0) distance _start < 300) exitWith {if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};{_nul = [_x] SPAWN SAOKSHF18;} foreach (units (_chop select 2));{if (vehicle _x == (_chop select 0)) then { unassignvehicle _x; _x action ["Eject",vehicle _x];};} foreach units _groupCT;DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];CantCommand = CantCommand - [_chop select 2];if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};FriendlyInf set [count FriendlyInf,_groupCT];};
...
if (isNil"_chop" || {isNil"_groupCT"}) exitWith {DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];};
if !((_chop select 0) distance _LZ < 700) exitWith {if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};{_nul = [_x] SPAWN SAOKSHF18;} foreach (units (_chop select 2));{if (vehicle _x == (_chop select 0)) then { unassignvehicle _x; _x action ["Eject",vehicle _x];};} foreach units _groupCT;DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];CantCommand = CantCommand - [_chop select 2];if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};FriendlyInf set [count FriendlyInf,_groupCT];};
//CREATE PAD
...
SAOKSHF23 = {
		...
		while {{(vehicle _x) distance _pos < 400} count ([] CALL AllPf) > 0 && {{isnull _x} count _bis_crows == 0}} do {sleep 5;};
		{if (!isnull _x) then {deletevehicle _x;};sleep 0.01;} foreach _bis_crows;
			{if (isNil"_x" || {typename _x != "ARRAY"} || {_x distance _pos < 10}) exitWith {CROWCOUNT = [CROWCOUNT,_forEachIndex] call BIS_fnc_removeIndex;};sleep 0.01;} foreach CROWCOUNT;
		};
	};
...
SAOKSHF24 = {
		...
		while {{(vehicle _x) distance _pos < 100} count ([] CALL AllPf) > 0 && {{isnull _x} count _bis_crows == 0}} do {sleep 5;};
		{if (!isnull _x) then {deletevehicle _x;};sleep 0.01;} foreach _bis_crows;
			{if (isNil"_x" || {typename _x != "ARRAY"} || {_x distance _pos < 10}) exitWith {FLYCOUNT = [FLYCOUNT,_forEachIndex] call BIS_fnc_removeIndex;};sleep 0.01;} foreach FLYCOUNT;
		};
};

SAOKSHF25 = {
	sleep 30; waituntil {sleep 15; isNull (_this select 0) || {{vehicle _x distance (_this select 0) < 500} count ([] CALL AllPf) == 0}}; if (!isNull (_this select 0)) then {[(_this select 0)] SPAWN FUNKTIO_DELUNIT;};
};
