...
if ({getmarkercolor _x == "ColorRed"} count AmbientZonesN > 0) then {_tasks = _tasks + [["MilitaryTasks\TaskAttackCamp.sqf","Capture CSAT Camp"]];};
if ({_x distance player < 4000 && {getmarkercolor (_x getvariable "Gmark") == "ColorGreen"} && {isNil{_x getvariable "Taken"}}} count GuardPosts > 0) then {_tasks = _tasks + [["MilitaryTasks\TaskGuardRoadBlock.sqf","Guard Roadblock"]];};
