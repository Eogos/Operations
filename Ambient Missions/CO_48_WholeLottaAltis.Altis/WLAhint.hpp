	...
	class WLA
	{
		...
		class WLANotes
		{
           ...
           // Optional hint subtitle, filled by arguments from 'arguments' param
			...
			// Structured text, filled by arguments from 'arguments' param
			description = "MISSION IS WORK IN PROGRESS";
                        // Optional structured text, filled by arguments from 'arguments' param (first argument
                        // is %11, see notes bellow), grey color of text
            tip = "%3If you end up enjoying this game mode, consider supporting and voting Whole Lotta Altis SP in MakeArmaNotWar.com competition%4";
			arguments[] = {};
			// Optional image
		...
		class GetStarted
		{
           ...
           // Optional hint subtitle, filled by arguments from 'arguments' param
			//displayNameShort = "Activate mode with ALT+Y while in any house. Exit by visiting friendly camp or return to location where mode was activated";
			// Structured text, filled by arguments from 'arguments' param
			description = "%2When dieing look for unit spawns in the menu to get fast together<br/>%2Leaning forward bring you faster down if there is enemies shooting at you when paragliding<br/>%2Use %3FAST-TRAVEL%4 to get quickly to your team-mates<br/>%2Team-leader can enable custom buttons for team-mates to buy vehicles/call support and construct<br/>%2If low on prestige, see houses from items (Y-key when near item)<br/>%2For more tips see briefing by clickin map open with M";
                        // Optional structured text, filled by arguments from 'arguments' param (first argument
                        // is %11, see notes bellow), grey color of text
            tip = "";
			arguments[] = {};
		...
		class IslandInsertion
		{
			// Hint title, filled by arguments from 'arguments' param
			displayName = "Island Insertion";
           // Optional hint subtitle, filled by arguments from 'arguments' param
			//displayNameShort = "Activate mode with ALT+Y while in any house. Exit by visiting friendly camp or return to location where mode was activated";
			// Structured text, filled by arguments from 'arguments' param
			description = "While on the respawn island and having %3ARSENAL/Armed Support%4 value over 0, you are able select your gear using %3VAS%4 via %3SHIFT+1%4<br/><br/>Once ready, walk away to be moved into selected respawn point or use %3FAST-TRAVEL%4 for custom destination<br/><br/>%3ISLAND INSERTION%4 can be disabled from %3WLA-menu%4 (SHIFT+1)";
                        // Optional structured text, filled by arguments from 'arguments' param (first argument
                        // is %11, see notes bellow), grey color of text
            tip = "Team-members are allowed to fast-travel into their team-leader location";
			arguments[] = {};
			// Optional image
			image = "\a3\ui_f\data\gui\cfg\hints\Miss_icon_ca.paa"; 
			// Hint scope, values as everywhere else in game. 2 means displayed, 0 and 1 not.)
		...
		class CallSupport
		{
           ...
           // Optional hint subtitle, filled by arguments from 'arguments' param
			//displayNameShort = "Activate mode with ALT+Y while in any house. Exit by visiting friendly camp or return to location where mode was activated";
			// Structured text, filled by arguments from 'arguments' param
			description = "%3Call Support%4-function is entered via WLA menu (%3Shift+1%4). Depending on progress in tasks, more options become available.<br/><br/>%2Called land support can be commanded with High Command-interface (%3Ctlr+Space%4)<br/>%2Once left much behind called Land and Air-vehicles create new zone on map that can be commanded on map view, and once reaching player again from High Command-interface";
                        // Optional structured text, filled by arguments from 'arguments' param (first argument
                        // is %11, see notes bellow), grey color of text
            tip = "If having antenna built in guardpost, support call prices are dropped by half near the post";
			arguments[] = {};
			// Optional image
			image = "\a3\ui_f\data\gui\cfg\hints\Miss_icon_ca.paa";
			// Hint scope, values as everywhere else in game. 2 means displayed, 0 and 1 not.)
		...
		class CustomSaveGame
		{
           ...
           // Optional hint subtitle, filled by arguments from 'arguments' param
			//displayNameShort = "Activate mode with ALT+Y while in any house. Exit by visiting friendly camp or return to location where mode was activated";
			// Structured text, filled by arguments from 'arguments' param
			description = "To continue your progress after mission or game updates, you need to use %3Savegame-button in WLA-menu (Shift+1)%4 or %3 0-0-0%4 to save.<br/><br/>Overall mission progress can be then resumed after restarting the mission - by leaving progress-setting, in the mission start options, to default resume-setting";
                        // Optional structured text, filled by arguments from 'arguments' param (first argument
                        // is %11, see notes bellow), grey color of text
		...
		class Construct
		{
           ...
           // Optional hint subtitle, filled by arguments from 'arguments' param
			...
			description = "You can use construct trucks/choppers to build own guardposts by hitting %3Shift+C%4 near the vehicle. First lay the fortress, after that more options appear. You can also place AI guards, weapons and vehicles after commiting certain tasks";
                        // Optional structured text, filled by arguments from 'arguments' param (first argument
                        // is %11, see notes bellow), grey color of text
            tip = "To get more construction vehicles, visit Other-sectio in %3Call Support%4-menu or %3Mil Shop%4 when near camp or marked airfield via %WLA menu (%3Shift+1%4)";
			arguments[] = {};
			// Optional image
			image = "\a3\ui_f\data\gui\cfg\hints\Miss_icon_ca.paa";
			// Hint scope, values as everywhere else in game. 2 means displayed, 0 and 1 not.)
		...
		class CustomMods
		{
           ...
           // Optional hint subtitle, filled by arguments from 'arguments' param
			//displayNameShort = "Activate mode with ALT+Y while in any house. Exit by visiting friendly camp or return to location where mode was activated";
			// Structured text, filled by arguments from 'arguments' param
			description = "Mod content can be enabled in WLA-menu (%3Shift+1%4) to make them appear during mission.<br/><br/>Virtual Battles have more random results when addon vehicles are taking part in it.<br/><br/>Keep %3at least one%4 class in each section to prevent errors and only use %3working classnames%4";
                        // Optional structured text, filled by arguments from 'arguments' param (first argument
                        // is %11, see notes bellow), grey color of text
		...
		class CustomSounds
		{
           ...
           // Optional hint subtitle, filled by arguments from 'arguments' param
			//displayNameShort = "Activate mode with ALT+Y while in any house. Exit by visiting friendly camp or return to location where mode was activated";
			// Structured text, filled by arguments from 'arguments' param
			description = "Custom voices and music are added into mission as %3separate addon%4.<br/><br/>You can install it from %3withSix%4 by searching WLA sound addon, manually from %3BI forum%4 mission topic named [SP/MP] Dynamic Whole Map ArmA3 Missions by SaOk. Later also from %3steam workshop%4 will have it.<br/><br/>Having sounds in separate addon allows smaller mission updates and sharing the sounds with other same concept missions";
                        // Optional structured text, filled by arguments from 'arguments' param (first argument
                        // is %11, see notes bellow), grey color of text
            tip = "Special Thanks to Zafjr (Soldiers, Civs, Surrendered), Jin (Player, Civs) and [FR] Helios (Music)";
			arguments[] = {};
			// Optional image image = "\a3\ui_f\data\gui\cfg\hints\Miss_icon_ca.paa"; image = "\a3\ui_f\data\gui\cfg\hints\VehicleGUI_ca.paa";
			image = "\a3\ui_f\data\gui\cfg\hints\VehicleGUI_ca.paa";
			// Hint scope, values as everywhere else in game. 2 means displayed, 0 and 1 not.)
