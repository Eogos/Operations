...
if (isNil{player getvariable "DrawFF"}) exitWith {};
_p = player getvariable "DrawFF";
...
_post setvariable ["StaticO",(_earlier + _l),true];
_earlier = +(_post getvariable "StaticO");
_r = count _earlier;
_li = [];
{
...
player setvariable ["DrawFF",nil,true];
[[-_c, "Construction", side player],"PRESTIGECHANGE",nil,false] spawn BIS_fnc_MP;
_n = [side player,(-1*_c)] SPAWN PrestigeUpdate;
} else {
