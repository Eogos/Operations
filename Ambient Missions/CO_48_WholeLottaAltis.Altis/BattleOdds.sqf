...
_type = 4;
_streght = [0.01,0.01,0.01,0.01,0.01,0.1,0.1,0.1,0.1];
_icon = "";
_name = "";
switch (_this) do {
//SPEEDBOATS
...
//"Greyhawk UAV"
case "O_UAV_02_CAS_F": {_type = 1;_streght = [1,1,8,8,8,1,1,8,8];};
case "I_UAV_02_CAS_F": {_type = 1;_streght = [1,1,8,8,8,1,1,8,8];};
case "B_UAV_02_CAS_F": {_type = 1;_streght = [1,1,8,8,8,1,1,8,8];};
//"Blackfoot"
case "B_Heli_Attack_01_F": {_type = 0;_streght = [5,2,7,8,9,5,5,9,9];};
case "O_Heli_Attack_02_F": {_type = 0;_streght = [5,2,7,8,9,5,5,9,9];};
case "O_Heli_Attack_02_black_F": {_type = 0;_streght = [5,2,7,8,9,5,5,9,9];};
//"Littlebird"
case "B_Heli_Light_01_armed_F": {_type = 0;_streght = [2,1,2,3,5,5,5,2,5];};
//"Buzzard AA"
case "I_Plane_Fighter_03_AA_F": {_type = 1;_streght = [7,7,4,4,2,2,2,1,2];};
//"Buzzard CAS"
case "I_Plane_Fighter_03_CAS_F": {_type = 1;_streght = [2,2,8,8,9,2,1,9,9];};

case "O_Plane_CAS_02_F": {_type = 1;_streght = [4,4,9,10,10,3,1,9,9];};

case "B_Plane_CAS_01_F": {_type = 1;_streght = [3,3,10,10,10,3,1,9,9];};
...
//"Cheetah"
case "B_APC_Tracked_01_AA_F": {_type = 3;_streght = [10,5,3,7,5,5,5,4,5];};
case "O_APC_Tracked_02_AA_F": {_type = 3;_streght = [10,5,3,7,5,5,5,4,5];};
//"Gordon"
...
//"Scorcher"
case "B_MBT_01_arty_F": {_type = 7;_streght = [1,1,5,5,6,7,7,3,7];};
case "O_MBT_02_arty_F": {_type = 7;_streght = [1,1,5,5,6,7,7,3,7];};
//"Sandstorm"
case "B_MBT_01_mlrs_F": {_type = 6;_streght = [1,1,5,5,6,7,7,3,7];};
//"Mortar"
...
if ({{_this in _x} count _x > 0} count ARMEDVEHICLES > 0) then {_type = 3;_streght = [4,1,1,4,7,7,7,2,7];};
if ({{_this in _x} count _x > 0} count ARMEDTANKS > 0) then {_type = 2;_streght = [3,1,7,9,9,5,4,9,9];};
if ({{_this in _x} count _x > 0} count ARMEDAA > 0) then {_type = 3;_streght = [10,5,3,7,5,5,5,4,5];};
if ({{_this in _x} count _x > 0} count AIRFIGTHER > 0) then {_type = 1;_streght = [2,2,8,8,9,2,1,9,9];};
if ({{_this in _x} count _x > 0} count AIRARMCHOP > 0) then {_type = 0;_streght = [5,2,7,8,9,5,5,9,9];};
};

if (count _streght == 0) then {_streght = [0.01,0.01,0.01,0.01,0.01,0.1,0.1,0.1,0.1];};
//if (_type == -1) then {hint format ["%1"]}
[_type,_streght,_icon,_name]