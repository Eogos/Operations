...
_dd = (["Lc"] CALL DIS);
_start = [(vehicle (([] CALL AllPf) call RETURNRANDOM)),_dd] CALL SAOKSEARCHPOS;
...
_dd = (["Lc"] CALL DIS);
_end = [(vehicle (([] CALL AllPf) call RETURNRANDOM)),_dd] CALL SAOKSEARCHPOS;
...
(_t1 select 2) SPAWN SAOKREMOVEWAYPOINTS; 
...
_aika = time + 300;
waitUntil {sleep 10;isNil"_t1" || {isNull (_t1 select 0)} || {((_t1 select 0) distance (_end)) < 50} || {!(alive leader (_t1 select 2))} || {{(vehicle _x == (_t1 select 0))} count ([] CALL AllPf) > 0} ||{!(alive (_t1 select 0))}|| {_aika < time} || {({(vehicle _x distance (_t1 select 0)) < 2200} count ([] CALL AllPf) == 0)}};
if (isNil"_t1" || {isNull (_t1 select 0)}) exitWith {};
if ({(vehicle _x == (_t1 select 0))} count ([] CALL AllPf) >0) exitWith {};
...
while {!isNil"_t1" && {!isNull (_t1 select 0)} && {({vehicle _x distance (_t1 select 0) < 500} count ([] CALL AllPf) > 0)}} do {
if ({(vehicle _x == (_t1 select 0))} count ([] CALL AllPf) > 0) exitWith {};
_end = [] CALL _e;
while  {surfaceIsWater _end} do {
sleep 5;
...
(_t1 select 2) SPAWN SAOKREMOVEWAYPOINTS; 
_wp1= (_t1 select 2) addWaypoint [_end, 0]; 
waitUntil {sleep 10; isNil"_t1" || {isNull (_t1 select 0)} || {((_t1 select 0) distance (_end) < 50)} || {!(alive leader  (_t1 select 2))} || {{(vehicle _x == (_t1 select 0))} count ([] CALL AllPf) > 0} || {!(alive (_t1 select 0))}};
...
if ({_x distance (_t1 select 0) < 500} count ([] CALL AllPf) == 0) then {
{unassignvehicle _x;[_x] ordergetin false;sleep 0.1;} foreach units  (_t1 select 2);
{deletevehicle _x;sleep 0.1;} foreach [(_t1 select 0)];
{deletevehicle _x;sleep 0.1;} foreach (units (_t1 select 2));
};
