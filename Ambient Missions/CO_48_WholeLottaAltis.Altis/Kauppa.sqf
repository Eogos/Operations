...
if (_class == "") then {
{
{if (_this == getText (configfile >> "CfgVehicles" >> _x >> "displayName")) exitWith {_class = _x;};} foreach ((AIRARMCHOP select _x)+(AIRFIGTHER select _x));
if (_class != "") exitWith {};
} foreach [0,1,2];
};
if (_class == "") then {
{
{if (_this == getText (configfile >> "CfgVehicles" >> _x >> "displayName")) exitWith {_class = _x;};} foreach ((ARMEDVEHICLES select _x)+(ARMEDTANKS select _x));
if (_class != "") exitWith {};
} foreach [0,1,2,3];
};
if (_class == "") then {
...
if (vehicle player distance (getmarkerpos (["ColorBlue"] CALL NEARESTCAMP)) < 100) then {
...
_t = if (isNil"IFENABLED") then {"CSAT Land Vehicles"} else {"Soviet Land Vehicles"};
lbAdd [1501, _t];
_t = if (isNil"IFENABLED") then {"CSAT Air Vehicles"} else {"Soviet Air Vehicles"};
lbAdd [1501, _t];
};
lbSetCurSel [1501, 0];
