...
_classes = [];
while {_random > 0} do {_classes pushBack (_ucc select (floor(random (count _ucc))));_random = _random - 1;};
_group = [_start, WEST, _classes,[],[],[0.4,0.8]] call SpawnGroupCustom;
if (!isNil"_group" && {count units _group > 0}) then {_unts = _unts + (units _group);_nul = [units _group] SPAWN AICampBehaviour;};
...
while {{vehicle _x distance _start < 400} count ([] CALL AllPf) > 0 || {surfaceIsWater _start} || {{_start distance _x < 400} count VarBlackListF > 0}} do {_start = [(_start select 0) + _range - (random (_range*2)),(_start select 1) + _range - (random (_range*2)),0];};
_random = round(random (_unitrate select 1));
while {_random <  (_unitrate select 0)} do {_random = round(random (_unitrate select 1));};
_classes = [];
while {_random > 0} do {_classes pushBack (_ucc select (floor(random (count _ucc))));_random = _random - 1;};
...
_dd = (["Lc"] CALL DIS);
{if ({vehicle _x distance _x < _dd} count ([] CALL AllPf) == 0) then {_objs = _objs - [_x];deletevehicle _x;};sleep 0.01;} foreach _objs; 
sleep 0.1;
{if (isnull _x) then {_objs = _objs - [_x];};sleep 0.01;} foreach _objs; 
sleep 0.1;
{_u = _x; if ({vehicle _x distance _u < _dd} count ([] CALL AllPf) == 0) then {CAMPUNITS = CAMPUNITS - [_x];deletevehicle _x;};sleep 0.01;} foreach CAMPUNITS;
sleep 0.1; 
{if (isnull _x) then {CAMPUNITS = CAMPUNITS - [_x];};sleep 0.01;} foreach CAMPUNITS; 
sleep 0.1;
{if (isNil"_x") exitWith {CAMPUNITS = [CAMPUNITS,_forEachIndex] call BIS_fnc_removeIndex;};sleep 0.01;} foreach CAMPUNITS;
sleep 0.1;
{if (isNil"_x") exitWith {_objs = [_objs,_forEachIndex] call BIS_fnc_removeIndex;};sleep 0.01;} foreach _objs;
sleep 10;
};
sleep 10;
//SEARCH GOOD PLACE
...
while {{(vehicle _x) distance _start < 400} count ([] CALL AllPf) > 0 || {(isOnRoad _start)} || {(_normal select 2 < 0.98)} || {_start CALL IORW} || {_start distance ([_start] CALL NEARESTGUARDPOST) < 800} || {surfaceIsWater _start} || {{side _x == EAST} count (_start nearEntities [["Man"],400]) > 0}} do {
_start = [] CALL _P;
_normal = surfaceNormal _start; 
_startS = _start CALL SAOKATLTOASL;
sleep 5;
...
CAMPUNITS = CAMPUNITS + (_d select 0);
_objs = _objs + (_d select 1);
};
_d = _start CALL _cUn;
CAMPUNITS = CAMPUNITS + _d;
