...
_okPosts = [];
{if (getmarkercolor (_x getvariable "Gmark") == "ColorGreen" && {[_x] CALL SAOKHAVECELL}) then {_okPosts pushBack _x;};} foreach GuardPosts;
...
} else {SaOkmissionnamespace setvariable ["Arrow",nil];};
};
SAOKCONSREMW = {
...
_obj setpos [(_pos select 0)+(_wTm select 0),(_pos select 1)+(_wTm select 1),_h];
...
if (isNil"_xx") then {
_a = _a + 1;
_dat = [_dat,_i] call BIS_fnc_removeIndex;
} else {
[_xx,_post, getmarkercolor _marker] CALL _f1;
};
};
if (_a > 0) then {_post setvariable ["StaticO",_dat,true];};
};
};
...
if (_a > 0) then {_post setvariable ["StaticOS",_dat,true];};
};
};
{
...
_u = [];
{
if (!isNil{_post getvariable _x}) then {_u = _u + ([_post,_x] CALL SAOKCONSADDU);};
...
if ({vehicle _x distance _postP < 450 && {isNil{_x getvariable "JustTeleported"}}} count ([] CALL AllPf) > 0) then {
...
//REM ONLY VEH
if (!isNil{(_post getvariable "WeaponsNow")}) then {
{{if (alive _x && {{isPlayer _x} count units group _x == 0}) then {deletevehicle _x;};} foreach crew ((_x select 0) select 1); deletevehicle ((_x select 0) select 1);} foreach (_post getvariable "WeaponsNow");
...
if ((_thism select 0) in ["Land_BarGate_F"]) then {
	...
	_ar = [CIVILIAN, WEST];
	if (_this select 2 == "ColorRed") then {_ar = [CIVILIAN, EAST];};
	_obj setvariable ["PassSide",_ar,true];
	//ANIMATED BAR GATE
	_nul = _obj SPAWN SAOKSHF8;
	///////////////////
};
...
_post setvariable ["Gmark",_mar,true];
};
_marker = _post getvariable "Gmark";
if (!isNil{(_post getvariable "ObjectsNow")}) then {
{deletevehicle (_x select 0);} foreach (_post getvariable "ObjectsNow");
...
if ({alive _x && {isNil{_x getvariable "SaOkSurrendered"}}} count _units == 0 && {{_x distance _post < _dd && {((getposATL _x) select 2) < 3}} count ([] CALL AllPf) > 0} && {_sidee == EAST}) then {
_bol = true;
};
_bol 
};
...
_bol = false;
_post = _this;
if ({_x distance _post < (["Lb"] CALL DIS)} count ([] CALL AllPf) == 0) then {_bol = true;};
_bol 
};
//_POST
...
if (isNil{_y getvariable "Gmark"}) then {
NUMM=NUMM+1;
_mar = format ["Guard%2PMM%1",NUMM,floor (random 10)];
_col = "ColorGreen";
_typ = "n_installation";
_marker = createMarker [_mar,getposATL _y];
_marker setMarkerShape "ICON";
_marker setMarkerType _typ;
_marker setMarkerSize [0,0];
_marker setMarkerColor _col;
_marker setMarkerText "";
_y setvariable ["Gmark",_mar,true];
};
...
sleep 0.5;
{if (isNil"_x" || {isNull _x}) exitWith {activatedPost = [activatedPost,_foreachIndex] call BIS_fnc_removeIndex;};sleep 0.1;} foreach activatedPost;
sleep 0.5;
_copy = + activatedPost;
_c = count _copy - 1;
if (_c > -1) then {
