private ["_class","_posPl","_raa","_xx","_yy","_start","_tank","_tg1","_tg1wp1","_wp1","_group2","_group3","_random","_pPosi","_classes","_nul","_roads","_flare","_sPosi","_st","_n","_ran","_distKM"];
if ({(getmarkerpos (["ColorBlue"] CALL NEARESTCAMP) distance vehicle _x < 2000)} count ([WEST] CALL AllPf) == 0) exitWith {};
if (random 1 < 0.5 && {"USHelp" in (SaOkmissionnamespace getvariable "Progress")}) then {
...
_pPosi = getposATL (vehicle (([WEST] CALL AllPf)call RETURNRANDOM));
_nul = [getmarkerpos "InsertionM", [(_pPosi select 0)+ 500 - (random 1000),(_pPosi select 1)+ 500 - (random 1000),0], [(_pPosi select 0)+ 500 - (random 1000)*0.5,(_pPosi select 1)+ 500 - (random 1000)*0.5,0], WEST, 180, _tank, 50,_classes,[0.6,0.7],0,1] SPAWN FChopperTransportF;
};

_distKM = (getmarkerpos (["ColorBlue"] CALL NEARESTCAMP) distance vehicle (([WEST] CALL AllPf)call RETURNRANDOM))*0.001;
sleep (10+(random 20)*_distKM);
//US Vehicles
if (random 10 < VarFS && {count FriendlyInf < FINFMAX}) then {
_xx = random 500;
_yy = 500 - _xx;
if (random 1 < 0.5) then {_xx = _xx*(-1)};
_yy = _yy*(-1);
_pPosi = getposATL (vehicle (([WEST] CALL AllPf)call RETURNRANDOM));
_sPosi = getmarkerpos (["ColorBlue"] CALL NEARESTCAMP);
...
while  {{_start distance vehicle _x < 1000} count ([WEST] CALL AllPf) > 0 || {surfaceIsWater _start} || {{_start distance _x < 500} count VarBlackListF > 0}} do {
sleep 5;
_xx = random (1100);
_yy = 1100 - _xx;
if (random 1 < 0.5) then {_xx = _xx*(-1)};
...
if (!("USHelp" in (SaOkmissionnamespace getvariable "Progress")) || {random 1 < 0.3}) then {{_nul = [_x,[]] SPAWN ConvertToArmedCivilian;} foreach units _group2;};
...
waituntil {sleep 1; behaviour leader _carg == "COMBAT" || {isNull _group} || {isNull _carg}}; {unassignvehicle _x; [_x] ordergetin false;} foreach units _group;
if (!isNull _carg) then {_nul = [_carg, getmarkerpos "Zone11",0] SPAWN FUNKTIO_MAD;};
};
...
_distKM = (getmarkerpos (["ColorBlue"] CALL NEARESTCAMP) distance vehicle (([WEST] CALL AllPf)call RETURNRANDOM))*0.001;
sleep (10+(random 20)*_distKM);
if (random 20 < VarFS && {count FriendlyVehicles < FVEHMAX} && {("GreenHelp" in (SaOkmissionnamespace getvariable "Progress"))}) then {
_xx = random 500;
_yy = 500 - _xx;
if (random 1 < 0.5) then {_xx = _xx*(-1)};
_yy = _yy*(-1);
...
if ({(getmarkerpos (["ColorBlue"] CALL NEARESTCAMP) distance vehicle _x < 2000)} count ([WEST] CALL AllPf) == 0) exitWith {};
//Air
if (VarAIRF && {random 15 < VarFS} && {"GreenHelp" in (SaOkmissionnamespace getvariable "Progress")}) then {
if (random 1 < 0.5 || {!("USHelp" in (SaOkmissionnamespace getvariable "Progress"))}) then {
...
_sPosi = getmarkerpos (["ColorBlue"] CALL NEARESTCAMP);
_start = [(_sPosi select 0) + _xx,(_sPosi select 1) + _yy,0];
_st = [_start, 400,"(1 + forest) * (1 - sea)",1] CALL FUNKTIO_POS;
_start = (_st select 0) select 0;
sleep 0.1;
_roads = (_start nearRoads 650);
if (count _roads > 0) then {_start = getposATL (_roads select 0);};  
while {{_start distance vehicle _x < 600} count ([WEST] CALL AllPf) > 0 || {surfaceIsWater [_start select 0, _start select 1]} || {{_start distance _x < 500} count VarBlackListF > 0}} do {
sleep 5;
_xx = random (800);
_yy = 800 - _xx;
if (random 1 < 0.5) then {_xx = _xx*(-1)};
...
{_nul = [_x,[]] SPAWN ConvertToArmedCivilian;} foreach units _group3;
_posPl = [(_pPosi select 0) +100-(random 200), (_pPosi select 1)+100-(random 200), 0];
