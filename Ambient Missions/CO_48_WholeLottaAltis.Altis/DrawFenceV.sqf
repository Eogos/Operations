...
_dir = ([_start, _end] call BIS_fnc_dirTo) +_d;
_dirO = _dir + 90;
_objs = [];
_lastPos = _start;
...
//if (_gPper <= _tt && {([_start] CALL RETURNGUARDPOST) distance _start > 120}) then {_GPos = [(_start select 0)+((sin (_dir + 90))*60),(_start select 1)+((cos (_dir + 90))*60),0]; _nul = [_GPos,"",25] execVM "CreateRoadBlock.sqf";_tt = 0;};
// [_clear, getpos _x] call BIS_fnc_inTrigger
