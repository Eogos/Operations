...
_civ = player;
{if (!isPlayer _civ && {_x distance player < 7} && {alive _x}) then {if (!(_x in CIVIGNORE)) then {[_x] SPAWN FUNKTIO_IGNORETIMED;}; _civ = _x; dostop _x;_x dowatch player; _x dotarget player; _list = _x; [_x] SPAWN {(_this select 0) setvariable ["Talki",1,true];sleep 120;(_this select 0) dofollow (_this select 0);(_this select 0) setvariable ["Talki",nil,true];};};} foreach _unts; 
...
if !("MetResContact" in (SaOkmissionnamespace getvariable "Progress")) then {
...
waitUntil {sleep 0.1; player kbWasSaid [_civ, "PlaV", (player getvariable "LastW"), 3] || {_aika < time}};
} else {sleep 4;};
