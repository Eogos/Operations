...
while {{_start distance (getmarkerpos _x) < (getmarkersize _x select 0)} count Bpositions > 0 || {{vehicle _x distance _start < 400} count ([] CALL AllPf) > 0} || {surfaceIsWater _start} || {{_start distance _x < 400} count VarBlackListE > 0}} do {_rP = ([] CALL AllPf) call RETURNRANDOM;_start = [(getposATL vehicle _rP select 0) + _range - (random (_range*2)),(getposATL vehicle _rP select 1) + _range - (random (_range*2)),0];};
_random = round(random (_unitrate select 1));
while {_random <  (_unitrate select 0)} do {_random = round(random (_unitrate select 1));};
_classes = [];
while {_random > 0} do {_classes pushBack (_ucc select (floor(random (count _ucc))));_random = _random - 1;};
...
//REMOVE UNNEEDED
_dd = (["Lc"] CALL DIS) + 100;
{_y = _x; if ({vehicle _x distance _y < _dd} count ([] CALL AllPf) == 0) then {deletevehicle _x;_CAMPOBJS = _CAMPOBJS - [_x];};sleep 0.1;} foreach _CAMPOBJS; 
sleep 0.1;
{if (isnull _x) then {_CAMPOBJS = _CAMPOBJS - [_x];};sleep 0.1;} foreach _CAMPOBJS; 
sleep 0.1;
{_y = _x; if ({vehicle _x distance _y < _dd} count ([] CALL AllPf) == 0) then {deletevehicle _x;_units = _units - [_x];};sleep 0.1;} foreach _units; 
sleep 0.1;
{if (isnull _x) then {_units = _units - [_x];};sleep 0.1;} foreach _units; 
sleep 0.1;
{if (isNil"_x") exitWith {_CAMPOBJS = [_CAMPOBJS,_forEachIndex] call BIS_fnc_removeIndex;};sleep 0.1;} foreach _CAMPOBJS;
sleep 0.1;
{if (isNil"_x") exitWith {_units = [_units,_forEachIndex] call BIS_fnc_removeIndex;};sleep 0.1;} foreach _units;
sleep 10;
};
sleep 10;
//SEARCH PLACE
...
while {{(vehicle _x) distance _start < 400} count ([] CALL AllPf) > 0 || {((getmarkerpos (["ColorRed",_start] CALL NEARESTCAMP) distance _start) > (getmarkerpos (["ColorBlue",_start] CALL NEARESTCAMP) distance _start))} || {(isOnRoad _start)} || {(_normal2 select 2 < 0.98)} || {_start CALL IORW} || {_start distance (getmarkerpos _x) < (getmarkersize _x select 0)} count Bpositions > 0  || {_start distance ([_start] CALL NEARESTGUARDPOST) < 800} || {surfaceIsWater _start} || {{[_startS,vehicle _x] CALL FUNKTIO_LOSOBJ} count ([] CALL AllPf) > 0} || {{side _x != EAST && {side _x != CIVILIAN}} count (_start nearEntities [["Man"],400]) > 0}} do {
_start = [] CALL _P;
_normal2 = surfaceNormal _start; 
_startS = _start CALL SAOKATLTOASL;
sleep 10;
