...
_nCb = getmarkerpos (["ColorBlue",_pos] CALL NEARESTCAMP);
_nCr = getmarkerpos (["ColorRed",_pos] CALL NEARESTCAMP);
if (_nG distance _pos < 50 || {_nCr distance _pos < 120} || {_nCb distance _pos < 120}) then {_bol = false;};
_bol
};
sleep 0.1;
if (count _waypoints > 0 && {{_x distance _hP < 30} count ([] CALL AllPf) == 0} && {_building CALL _ff} && {count (_hP nearEntities [["Civilian"], 10]) < 3} &&  {!(typeOf _building in _bLar)}) then {
sleep 0.1;
...
{if (count rCiv > _LL) then {rCiv = rCiv - [_x]; deletevehicle _x;};sleep 0.1;} foreach rCiv;
};
{if (isNull _x) exitWith {rCiv = rCiv - [_x];};sleep 0.1;} foreach rCiv;
{if (isNil"_x") exitWith {rCiv = [rCiv,_forEachIndex] call BIS_fnc_removeIndex;};sleep 0.1;} foreach rCiv;
_dd = (["C"] CALL DIS)*1.5;
{_y = _x; if ({_y distance vehicle _x < _dd} count ([] CALL AllPf) == 0) then {_x removeAllEventHandlers "Killed";rCiv = rCiv - [_x];deletevehicle _x;};sleep 0.1;} foreach rCiv;
_maxCiv = _LL;
if (!([] CALL FPSGOOD) || {{vehicle _x != _x} count ([] CALL AllPf) == 0}) then {_maxCiv = 7;};
