...
if (_side == EAST) then {[[_start],"mil_arrow","ColorRed",([_start, _LZ] call BIS_fnc_dirTo)] SPAWN SAOKCREATEMARKER;};
...
waitUntil {sleep 2; isNil"_chop" || {isNil"_groupCTP"} || {(_chop select 0) distance _LZ < 350} || {!(alive (_chop select 0))} || {!(alive driver (_chop select 0))} || {(getposATL (_chop select 0) select 2) < 1} || {{alive _x} count crew (_chop select 0) == 0}};
if (isNil"_chop" || {isNil"_groupCTP"}) exitWith {DONTDELGROUPS = DONTDELGROUPS -  [_groupCTP];};
if (!(alive (_chop select 0)) || {!(alive driver (_chop select 0))} || {(getposATL (_chop select 0) select 2) < 1} || {{alive _x} count crew (_chop select 0) == 0}) exitWith {if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};
{
_nul = [_x] SPAWN {
private ["_unit"];
...
} foreach units _groupCTP;

DONTDELGROUPS = DONTDELGROUPS -  [_groupCTP];};



{_nul = [_x,(_chop select 0)] SPAWN FJumpOff;} foreach units _groupCTP;
...
waitUntil {sleep 5; isNil"_chop" || {isNil"_groupCTP"} ||  {(_chop select 0) distance _start < 300} || {!(alive (_chop select 0))} || {!(alive driver (_chop select 0))} || {(getposATL (_chop select 0) select 2) < 1} || {{alive _x} count crew (_chop select 0) == 0}};
if (isNil"_chop" || {isNil"_groupCTP"}) exitWith {DONTDELGROUPS = DONTDELGROUPS -  [_groupCTP];};
if (!(alive (_chop select 0)) || {!(alive driver (_chop select 0))} || {(getposATL (_chop select 0) select 2) < 1} || {{alive _x} count crew (_chop select 0) == 0}) exitWith {if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};
{
_nul = [_x] SPAWN {
...
{deletevehicle _x;} foreach (units (_chop select 2));
{deletevehicle _x;} foreach [(_chop select 0)];
