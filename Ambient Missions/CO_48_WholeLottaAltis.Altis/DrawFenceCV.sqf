...
_mmm = _mmm + 1;
if (_objectType == "Land_Mil_WiredFenceD_F") then {_objectType = _this select 2;};
...
_datAr = _datAr + [[_objectType,_start, _ddd, (surfaceNormal _start),_id]];
} else {
...
} else {_hole = 0;_objectType = _orgT;_maxWidth = _orgW;};
_dirL = (_Sdir+_dir);
_nSd = 30 - random 60;
while {70 < (_Sdir + _nSd) || {-70 > (_Sdir + _nSd)}} do {_nSd = 30 - random 60;};
if (random 1 < 0.9) then {_Sdir = (_Sdir + _nSd);};
if (random 1 < 0.3 && {_mmm > 3} && {_bolFF}) then {_mmm = 0;_objectType = "Land_BagBunker_Small_F"; _maxWidth = ("Land_BagBunker_Small_F" CALL SAOKBSIZE);} else {
...
while {_dirE > 360} do {_dirE = _dirE - 360;};
*/
_start = [(_start select 0)+((sin _dirL)*_maxWidth*0.5)+((sin (_Sdir+_dir))*_maxWidth*0.5),(_start select 1)+((cos _dirL)*_maxWidth*0.5)+((cos (_Sdir+_dir))*_maxWidth*0.5),0];
/*
_dirEE = ([_end, _startN] call BIS_fnc_dirTo);
if (_dirEE < 0) then {_dirEE = 360 + _dirEE;};
_abs = abs(_dirEE - _dirE); 
while {!(_abs >= 30 && {_abs <= 330})} do {
_startN = [(_start select 0)+((sin _dirL)*_maxWidth*0.5)+((sin (_Sdir+_dir))*_maxWidth*0.5),(_start select 1)+((cos _dirL)*_maxWidth*0.5)+((cos (_Sdir+_dir))*_maxWidth*0.5),0];
_dirEE = ([_end, _startN] call BIS_fnc_dirTo);
if (_dirEE < 0) then {_dirEE = 360 + _dirEE;};
_abs = abs(_dirEE - _dirE); 
};
