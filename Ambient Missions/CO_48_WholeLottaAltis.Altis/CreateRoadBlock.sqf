...
//_post _class _pos _dir    _r = guardposts call RETURNRANDOM; _u = (_r getvariable "StaticO"); copyToClipboard format["%1",_u];
_ff = {
...
_wallD = _wallD + (_d select 2);
} else {
if (random 1 < 0.8) then {
_d = ([_R,_rad,_wallT,"",0] CALL FUNKTIO_DrawBoxV);
_wallD = _wallD + (_d select 1);
_c = count _wallD - 1;
_c = floor (random _c);
_wallD = [_wallD,_c] call BIS_fnc_removeIndex;
//_datAr = _datAr + [[_objectType,_start, _ddd, (surfaceNormal _start),_id]]; AOKADDGUNGENERATED
{if (_x select 0 == "Land_BagBunker_Small_F") then {_gunP set [count _gunP, _x]};} foreach _wallD;
_walls = _walls + (_d select 0);
{_pickedPosses set [count _pickedPosses, _x select 1];} foreach (_d select 1);
{_pickedPosses set [count _pickedPosses, getposATL _x];} foreach (_d select 0);
} else {
...
_pickedPosses set [count _pickedPosses, _p];
...
if (_type == "RES") then {_Tower setvariable ["RES",1,true];};
...
if ({!isNil{_x getvariable (_this select 3)}} count GuardPosts == 0) then {_post setvariable [(_this select 3),1,true];};
};
};
sleep 0.1;
...
_walls = _walls + (_d select 0);
_wallD = _wallD + (_d select 2);
{_pickedPosses set [count _pickedPosses, _x select 1];} foreach (_d select 2);
{_pickedPosses set [count _pickedPosses, getposATL _x];} foreach (_d select 0);
};
//_fsm setFSMVariable ["_ww", []];
sleep 0.1;
...
_clcl = ["Land_PaperBox_open_empty_F","Land_Pallet_MilBoxes_F","Land_PaperBox_closed_F","Land_CampingTable_small_F","Land_CampingTable_F","Land_Campfire_F","Campfire_burning_F","Land_WoodenLog_F","Land_WoodenTable_large_F","Land_WoodenTable_small_F","Land_Pallets_stack_F"];
{if (isClass(configFile >> "cfgVehicles" >> _x)) then {_clcl pushback _x};} foreach ["Land_DieselGroundPowerUnit_01_F","Land_JetEngineStarter_01_F","Land_PaperBox_closed_F"];
_clcl = _clcl call RETURNRANDOM;
...
_pickedPosses set [count _pickedPosses, _start];
_size = [0,1,2,3] call RETURNRANDOM;
_ang = random 360;
_ToP = + _start;
//_post _class _pos _dir
_wallD  = _wallD  + ([(count _wallD),_clcl,_start,_ang] CALL _ff);
...
_pickedPosses set [count _pickedPosses, _start];
_wallD  = _wallD  + ([(count _wallD),_benc,_start,_ang] CALL _ff);
};
};
...
_pickedPosses set [count _pickedPosses, _start];
if (_t < 8) then {_wallDS  = _wallDS  + ([(count _wallDS),_clcl,_start,_ang] CALL _ffS);};
};
/*
_size = [1,2] call RETURNRANDOM;
for "_i" from 0 to _size do {
_clcl = ["Land_MilOffices_V1_F"]call RETURNRANDOM;
_ang = random 360;
_radius = 50 + (random 30);
_start = [(_R select 0)+((sin _ang)*_radius),(_R select 1)+((cos _ang)*_radius),0];
_t = 0;
_sizeObj = (_clcl CALL SAOKBSIZE);
while {(count (_start nearObjects ["House",_sizeObj]) > 0 || {count (_start nearObjects ["Static",_sizeObj]) > 0} || {{_x distance _start < _sizeObj} count _pickedPosses > 0}) && {_t < 8}} do {
_t = _t + 1;
_ang = random 360;
_radius = 100;
_start = [(_R select 0)+((sin _ang)*_radius),(_R select 1)+((cos _ang)*_radius),0];
...
if (_rad > 70) then {
if (isNil"IFENABLED") then {
_arO = _arO  + ["Land_Communication_F","Land_Dome_Big_F","Land_Dome_Small_F","Land_Device_assembled_F","Land_Device_disassembled_F"];
};
};
if (_rad > 50) then {
if (!isNil"IFENABLED") then {
_arO = _arO  + ["trench_tank1","trench_big1","trench_mortar","trench_mg1","trench_mg2","trench_gun1","trench_single1","trench_small1","Land_TTowerSmall_2_F","Land_TTowerSmall_1_F"];
} else {
_arO = _arO  + ["Land_Ind_IlluminantTower","Land_Antenna","Land_Tryapka","Fuel_can","land_Staszow_map","Land_TTowerSmall_2_F","Land_TTowerSmall_1_F","Land_Medevac_HQ_V1_F","Land_Medevac_house_V1_F","Land_Device_assembled_F","Land_Device_disassembled_F"];
};
};
if (isNil"IFENABLED") then {
_arO = _arO  + ["Land_Medevac_house_V1_F","Land_Device_assembled_F","Land_Device_disassembled_F"];
};
_size = count _arO - 1; 
for "_i" from 0 to _size do {
...
if ([_start,_sizeObj2] call SAOKISFLAT) then {
_pickedPosses set [count _pickedPosses, _start];
if (_t < 14) then {_wallDS  = _wallDS  + ([(count _wallDS),_clcl,_start,_ang] CALL _ffS);};
};
sleep 0.1;
};
...
if (_rad > 70) then {
if (isNil"IFENABLED") then {
_arO = _arO  + ["Land_Cargo_Patrol_V3_F","Land_Cargo_Patrol_V2_F","Land_Cargo_Patrol_V1_F","Land_Cargo_House_V1_F","Land_Cargo_House_V1_F","Land_Cargo_House_V1_F","Land_Cargo_House_V1_F","Land_Cargo_House_V1_F","Land_Cargo_House_V1_F","Land_Cargo_HQ_V1_F","Land_Cargo_House_V1_F","Land_Cargo_House_V1_F","Land_PowerGenerator_F","Land_Loudspeakers_F","Land_FieldToilet_F","Land_Scaffolding_F","Land_CratesWooden_F","Land_CratesWooden_F"];
} else {
_arO = _arO  + ["trench_tank1","trench_big1","trench_mortar","trench_mg1","trench_mg2","trench_gun1","trench_single1","trench_small1","Land_Ind_IlluminantTower","Land_Antenna","Land_Tryapka","Fuel_can","land_Staszow_map","Land_PowerGenerator_F","Land_Loudspeakers_F","Land_Scaffolding_F","Land_CratesWooden_F","Land_CratesWooden_F"];
};
};
if (_rad > 50) then {
if (isNil"IFENABLED") then {
...
if (isNil"IFENABLED") then {
_arO = _arO  + ["Land_Cargo_House_V1_F","Land_PowerGenerator_F","Land_Loudspeakers_F","Land_Sink_F","Land_CampingTable_F","Land_FieldToilet_F","MapBoard_altis_F","Land_CratesWooden_F"];
};
_size = count _arO - 1; 
for "_i" from 0 to _size do {
_clcl = _arO select _i;
...
if ([_start,_sizeObj2] call SAOKISFLAT) then {
_pickedPosses set [count _pickedPosses, _start];
if (_t < 14) then {_wallD  = _wallD  + ([(count _wallD),_clcl,_start,_ang] CALL _ff);};
};
sleep 0.1;
};
...
_wallD  = _wallD  + ([(count _wallD),_x,_Tp,(random 360)] CALL _ff);
} foreach ["Land_TentA_F","Land_TentA_F","Land_TentA_F","Land_TentA_F"];

{if (isNil"_x" || {isNull _x}) exitWith {_walls = [_walls,_forEachIndex] call BIS_fnc_removeIndex;deletevehicle _x;};} foreach _walls;

_earlier = _post getvariable "StaticO";
_post setvariable ["StaticO",(_earlier + _wallD),true];
_earlier = _post getvariable "StaticOS";
_post setvariable ["StaticOS",(_earlier + _wallDS),true];
...
sleep 10;
if ({_x distance _post < (["Lb"] CALL DIS)} count ([] CALL AllPf) > 0) then {sleep 1;_post CALL SAOKCONSRESETOBJ;};
