...
if !((_chop select 0) distance _LZ < 350) exitWith {if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};{_nul = [_x] SPAWN SAOKSHF18;} foreach (units (_chop select 2));{if (vehicle _x == (_chop select 0)) then { unassignvehicle _x; _x action ["Eject",vehicle _x];};} foreach units _groupCT;DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];};
//ELSE NEAR LZ -> JUMP
...
//CHOP DOWN
if (isNil"_chop" || {isNil"_groupCT"}) exitWith {DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];};
if !((_chop select 0) distance _start < 300) exitWith {if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};{_nul = [_x] SPAWN SAOKSHF18;} foreach (units (_chop select 2));{if (vehicle _x == (_chop select 0)) then { unassignvehicle _x; _x action ["Eject",vehicle _x];};} foreach units _groupCT;DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];};
...
if (isNil"_chop" || {isNil"_groupCT"}) exitWith {DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];};
if !((_chop select 0) distance _LZ < 700) exitWith {if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};{_nul = [_x] SPAWN SAOKSHF18;} foreach (units (_chop select 2));{if (vehicle _x == (_chop select 0)) then { unassignvehicle _x; _x action ["Eject",vehicle _x];};} foreach units _groupCT;DONTDELGROUPS = DONTDELGROUPS -  [_groupCT];};
//CREATE PAD
