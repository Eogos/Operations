...
if (typename _this == "STRING") then {getmarkerpos _this} else {getposATL _this}
};
...
if (!isNil"SAOKFLPROG" || {isNil"StartMission"} || {count AmbientZonesN == 0}) exitWith {};
_factor = SAOKMAPSIZE / 30000;
SAOKFLPROG = 1;
sleep 10;
{deletemarker _x;} foreach SAOKFLMs;
SAOKFLMs = [];
_c = count AmbientZonesN - 1;
if (_c > 1) then {
for "_i" from 0 to _c do {
_camp = AmbientZonesN select _i;
...
_ma = 360 - _degrees;
while {_ang < _ma} do {
_nC = (["ColorBlue",_start] CALL NEARESTCAMP);
if (_nC == _camp) then {
...
if (_start distance _center > (getmarkerpos (["ColorRed",_start] CALL NEARESTCAMP)) distance _start) then {
_radi = ((_center distance _start) + (_start distance (getmarkerpos (["ColorRed",_start] CALL NEARESTCAMP)))) * 0.5;
...
for "_i" from 0 to _c do {
_gP = GuardPosts select _i;
if (getmarkercolor (_gP getvariable "Gmark") == "ColorGreen") then {
...
_ma = 360 - _degrees;
while {_ang < _ma} do {
_nC = getmarkerpos (["ColorBlue",_start] CALL NEARESTCAMP);
_nCr = getmarkerpos (["ColorRed",_start] CALL NEARESTCAMP);
...
//_dirTo = [_gP, _nGr] call BIS_fnc_dirTo;
_radi = ((_center distance _start) + (_start distance _nGr)) * 0.5;
_start2 = [(_center select 0)+((sin _ang)*_radi),(_center select 1)+((cos _ang)*_radi),0];
[_start2,_ang] SPAWN SAOKLINEMAR;
} else {
