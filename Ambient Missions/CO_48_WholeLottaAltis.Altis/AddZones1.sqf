...
waitUntil {sleep 5; count AmbientZonesN > 5};
waitUntil {sleep 1; !isNil"StartMission"};
sleep 5;
_g = [];
{if (getmarkercolor _x == "ColorBlue") then {_g pushBack (getmarkerpos _x);};} foreach AmbientZonesN;
...
if (!surfaceIsWater _star) then {
_n = ["N","AA",_star,1] SPAWN CVZ; _num = _num + 1;
};
sleep 1;
};
};
...
if (!surfaceIsWater _star) then {
_n = ["N","T",_star,2] SPAWN CVZ; _num = _num + 1;
};
sleep 1;
};
};
...
if (!surfaceIsWater _star) then {
_n = ["N","V",_star,2] SPAWN CVZ; _num = _num + 1;
};
sleep 1;
};
};
...
_n = [_star, "ColorRed",[_rC call RETURNRANDOM,_rC call RETURNRANDOM],"o_support"] CALL AddVehicleZone; _num = _num + 1;
};
sleep 1;
};
};
...
_star = [random SAOKMAPSIZE,random SAOKMAPSIZE,0];
if (!surfaceIsWater _star) then {
_n = ["H","AA",_star,1] SPAWN CVZ; _num = _num + 1;
};
sleep 1;
};
};