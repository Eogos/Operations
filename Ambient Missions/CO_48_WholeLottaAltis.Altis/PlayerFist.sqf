if (vehicle player != player || {currentWeapon player != ""} || {!isNil{player getvariable "Striking"}}) exitWith {};
player setvariable ["Striking",1];
_ani = "AwopPercMstpSgthWnonDnon_end";
_h = ((eyepos player) select 2)-((getposASL player) select 2);
_bol = false;
if (_h >= 1.4) then {
//player playmovenow _ani;
[[player,_ani],"SAOKPMOVE",nil,false] spawn BIS_fnc_MP;
...
{if (!isPlayer _x  && {!([player, _x] CALL FUNKTIO_LOS)} && {([_x, player] CALL FUNKTIO_LOS)}) exitWith {_x setdamage 1;[player,(_rs call RETURNRANDOM)] SPAWN SAOKSAY3D;_bol = true;};} foreach _u;
};
};
player setvariable ["Striking",nil];
sleep 0.5;
...
if (alive _xx && {side _x == EAST} && {!(_xx in SPWITHW)} && {[player, _xx] CALL FUNKTIO_LOS}) then {SPWITHW set [count SPWITHW,_xx];};
sleep 0.05;
};
};
