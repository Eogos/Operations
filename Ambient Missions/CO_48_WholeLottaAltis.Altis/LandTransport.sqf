...
sleep 15;
CARS set [count CARS,(_chop select 0)];
if (!_notBoat && {alive (_chop select 0)} && {(_chop select 0) distance _LZ < 20} &&  {{alive _x} count crew (_chop select 0) > 0}) then {_dir = [_LZ, _start] call BIS_fnc_dirTo; (_chop select 0) setdir _dir; (_chop select 0) setpos _LZ;};
(_chop select 2) move _start;(_chop select 0) forcespeed -1;
waitUntil{sleep 5; isNil"_chop" || {isNil"_groupLT"} || {isNull (_chop select 0)} || {!(alive (_chop select 0))} || {{vehicle _x distance (_chop select 0) < 2500} count ([] CALL AllPf) == 0} || {!(alive driver (_chop select 0))} || {({alive _x} count crew (_chop select 0)) < 1} || {(_chop select 0) distance _start < 40}};
if (isNil"_chop" || {isNil"_groupLT"}) exitWith {};
if (!(alive (_chop select 0)) || {!(alive driver (_chop select 0))} || {({alive _x} count crew (_chop select 0)) < 1}) exitWith {if (isNil("CARS")) then {} else {};{_nul = [_x] SPAWN {private ["_unit"];_unit = _this select 0;sleep 60;waituntil {sleep 10; {vehicle _x distance _unit < 700} count ([] CALL AllPf) == 0 || isNull _unit};deletevehicle _unit;};} foreach (units (_chop select 2));Pgroups set [count Pgroups,_groupLT];};
