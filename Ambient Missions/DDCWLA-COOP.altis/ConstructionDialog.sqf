...
if (!isNil{SaOkmissionnamespace getvariable "DrawFF"} && {(player distance ((SaOkmissionnamespace getvariable "DrawFF") select 1) > 100)}) then {
SaOkmissionnamespace setvariable ["DrawFF",nil];
};
...
sleep (random 0.05);
if (!isNil{SaOkmissionnamespace getvariable "CurObject"}) then {deletevehicle (SaOkmissionnamespace getvariable "CurObject");SaOkmissionnamespace setvariable ["CurObject",nil];};
if (!dialog && {isNil"LIIKUTAOBJEKTI"}) then {

if (!isNil{SaOkmissionnamespace getvariable "ConsMoved"} && {_post distance (getposATL player) > 50}) then {
if (0 != ((SaOkmissionnamespace getvariable "ConsMoved") select 1)) then {
...
_c = 0; 
if (!isNil{(SaOkmissionnamespace getvariable "LastCat")}) then {_c =(SaOkmissionnamespace getvariable "LastCat");};
lbSetCurSel [1501, _c];
SaOkmissionnamespace setvariable ["LastCat",_c];
(lbText [1501, (lbCurSel 1501)]) CALL SAOKCONSCATHEGORY;
_c = 0;
if (!isNil{ (SaOkmissionnamespace getvariable "LastOb")}) then {_c =(SaOkmissionnamespace getvariable "LastOb");};
lbSetCurSel [1500, _c];
SaOkmissionnamespace setvariable ["LastOb",_c];
