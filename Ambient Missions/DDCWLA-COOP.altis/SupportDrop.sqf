...
if (_class iskindof "LandVehicle") then {AddIdVeh set [count AddIdVeh, _roina];};
[_roina] SPAWN {
private ["_maxTime","_smoke","_roina"];
_roina = _this select 0;
sleep 5;
_maxTime = 14;
while {!isNil"_roina" && {!isNull _roina} && {{vehicle _x distance _roina < 15} count ([] CALL AllPf) == 0} && {_maxTime > 0}} do {
...
waitUntil {sleep 0.2;(getposATL _roina select 2) < 2 || {(getposASL _roina select 2) < 2}};
detach _roina;
sleep 1;
deletevehicle _veh;
...
waitUntil {sleep 30; {vehicle _x distance _roina < 700} count ([] CALL AllPf) == 0};
};
if (!isNull _roina) then {
