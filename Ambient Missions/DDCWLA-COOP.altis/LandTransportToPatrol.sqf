...
waitUntil {sleep 3; isNil"_chop" || {isNil"_groupLTTP"} || {isNull (_chop select 0)} || {(_chop select 0) distance _start < 40} || {{vehicle _x distance (_chop select 0) < 2500} count ([] CALL AllPf) == 0} || {!(alive (_chop select 0))} || {!(alive driver (_chop select 0))} || {({alive _x} count crew (_chop select 0)) < 1} || {behaviour leader (_chop select 2) == "COMBAT" && {_notBoat}}};
if (isNil"_chop" || {isNil"_groupLTTP"}) exitWith {DONTDELGROUPS = DONTDELGROUPS - [_groupLTTP];};
if (!(alive (_chop select 0)) || {!(alive driver (_chop select 0))} || {({alive _x} count crew (_chop select 0)) < 1}) exitWith {
DONTDELGROUPS = DONTDELGROUPS - [_groupLTTP];
if (isNil("CARS")) then {} else {CARS set [count CARS,(_chop select 0)];};
