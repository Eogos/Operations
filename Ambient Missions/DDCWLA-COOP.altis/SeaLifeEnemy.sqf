...
while {count _units > 4} do {sleep 7; {if (_x distance player > 220) then {_units = _units - [_x];deletevehicle _x;};} foreach _units;};
_Ppos = (getposATL player);
_start = [(_Ppos select 0) - 100 + random 200, (_Ppos select 1) - 100 + random 200, -10+(random 8)];
while {!surfaceIsWater _start || [_start,player] CALL FUNKTIO_LOSOBJ || _start distance player < 60} do {sleep 0.1;_start = [(_Ppos select 0) - 100 + random 200, (_Ppos select 1) - 100 + random 200, -10+(random 9)];}; 
