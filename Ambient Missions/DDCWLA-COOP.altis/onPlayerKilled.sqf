...
private ["_vI","_uI","_bp","_bC","_mag","_mag2","_we","_typeOf","_uni","_hat","_da","_Vest","_itP","_itS","_data","_curr_wpn","_killer","_dw_killer_distance","_dw_killer_skill","_dw_killer_skill_array","_dw_killer_combatmode","_dw_killer_behaviour","_dw_killer_actualWeapon","_dw_killer_zeroing","_dw_killer_vision","_dw_killer_precision","_dw_killer_weaponstate","_dw_killer_weaponsItems","_dw_killer_weaponAccessories","_dw_killer_unitPos","_dw_killer_health","_dw_killer_fatigue","_dw_killer_moral","_victim","_victim_unitPos","_dw_victim_unitPos","_dw_killer_canFire","_dw_killer_fleeing","_dw_killer_handshit","_dw_killer_getHit_head","_dw_killer_getHit_hands","_dw_rank_killer","_dw_rankID_killer","_dw_killer_simulationEnabled","_dw_killer_speed","_dw_killer_stance","_dw_killer_unitRecoilCoefficient","_dw_assignedTarget","_dw_wind"];
player setvariable ["Klocation", getposATL (_this select 0)];
if (!isNull (_this select 0) && {count weapons (_this select 0) > 1} && {primaryWeapon (_this select 0) != ""}) then {
_vI = vestItems (_this select 0);
_uI = uniformItems (_this select 0);
_bp = Backpack (_this select 0);
_bC = backpackItems (_this select 0);
_mag = primaryWeaponMagazine (_this select 0);
_mag2 = secondaryWeaponMagazine (_this select 0);
_we = + (weapons (_this select 0));
_typeOf = typeOf (_this select 0);
_uni = uniform (_this select 0);
_hat = headgear (_this select 0);
_da= damage (_this select 0);
_Vest = Vest (_this select 0);
_itP = if (!isNil{primaryWeaponItems (_this select 0)}) then {primaryWeaponItems (_this select 0)} else {[]};
_itS = if (!isNil{secondaryWeaponItems (_this select 0)}) then {secondaryWeaponItems (_this select 0)} else {[]};
_data = [_vI,_uI,_bp,_bC,_mag,_mag2,_we,_typeOf,_uni,_hat,_da,_Vest,_itP,_itS];
player setvariable ["GearData",_data];
} else {
player setvariable ["OwnRes",((player getvariable "OwnRes")+1),true];
player setvariable ["GearData",nil];
};
_pS = "ColorBlue";
if (side player != WEST) then {_pS = "ColorRed";};
if (count ([side player] CALL BIS_fnc_getRespawnPositions) < 3 && {!isNil"AmbientZonesN" && {({getmarkercolor _x == _pS} count AmbientZonesN == 0)}} && {!surfaceisWater (getposATL (_this select 0))}) then {
_nul = [side player,getposATL (_this select 0)] call BIS_fnc_addRespawnPosition;
};


//note: uses basic "Killed" EH, same possibility is "MPkilled" (server-side) //"HitPart" can provide quite interesting info too if needed
