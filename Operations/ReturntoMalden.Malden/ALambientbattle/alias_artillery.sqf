...
while {(al_art) and (!isNull _main_art_object)} do {
	_rel_Pos= [getPos _main_art_object,random _range_art, random 360] call BIS_fnc_relPos;;
	_art_object setPos _rel_Pos;
	//[[[_art_object],"ALambientbattle\alias_art_effect.sqf"],"BIS_fnc_execVM",true,false] spawn BIS_fnc_MP;
	[[_art_object],"ALambientbattle\alias_art_effect.sqf"] remoteExec ["BIS_fnc_execVM"];
	sleep 0.5+random 2;
};	